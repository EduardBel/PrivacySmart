async function manageCookies(vendor){  //detecció banner cookies
  switch(vendor){
    ...
    break;
    default: console.log("PrivacySmart: We are not dealing with a controlled vendor")
              return 0
    break;
  }
}
...
async function creaSC(ac){  //genera SC i fa deploy
  console.log("PrivacySmart: accessing SC documents...")
  // documents de l'SC
  ...
  .send({
    ...
    .then((newContractInstance) => {
      console.log("PrivacySmart: the transaction was successful!")
...
}catch(e){console.log("PrivacySmart: There has been some error related to the Blockchain connection. "+e)}
}
  ...
  if(typeof(wallet)!="undefined"){
    browser.storage.local.get(['preferencies']).then( //prenem el valor de preferències introduit per l'user
        async res =>{ preferencies=res.preferencies
          ...
          else console.log("PrivacySmart: if you want the plugin to manage you cookies you must input the preference level on the pop-up!")                       
        }); 
  }
  else console.log("PrivacySmart: if you want the plugin to manage you cookies you must input your wallet's address on the pop-up!")
}
...
async function waitForBanner() {
  await timeoutCall(list).then((successMessage) => {
    if(successMessage!=="not found"){
      console.log("PrivacySmart: we found a banner from " + successMessage);
      gotCookies(successMessage)
    }
    else console.log("PrivacySmart: we didn't find any cookie banner controlled by the plugin")
  });
